...
const decl l = 2;	// number of restrictions (\phi submodel) 

const decl  delta0 = <0,0>;   // value of the delta's under H0
...
main(){
	...
	decl betaols, etaols, muols, varols, phiols,deltaols, etaolsl, muolsl, varolsl, phiolsl,deltaolsl;
	decl LR, Skov1,Skov2;
	...
	s_rHl = s_rH[][0:(q-l-1)]; // matrix H (restricted)
	...
	varols = ((ys1-etaols)'*(ys1-etaols)) ./ ((nobs-p) .* ((1 ./(muols .*(1-muols))) .^2));
	...
	if(q>1){
		ols2c(log(phiols), s_rHl, &deltaolsl);
	}
	else if(q==1){
		deltaolsl = meanc(phiols);
	}
	   	 
	vtheta1 =  betaols | deltaolsl;  // starting values 
	...
	decl etatil = s_tX*(vtheta1[0:(p-1)]);
	decl mutil = exp(etatil) ./ (1+exp(etatil));
	decl T1til = diag(mutil .* (1-mutil));
	decl zetatil = s_rH*(vtheta1[p:(p+q-1-l)] |delta0');
	...
	println("\nNull hypothesis H0: (delta_",(q-l+1), " = ... = delta_",q,") = 0");
